...
/*
 ...
 +-------------------------------------------------------------------------+
 | Cacti: The Complete RRDtool-based Neighboring Solution                     |
 +-------------------------------------------------------------------------+
...
function save() {
	if (isset_request_var('save_component_neighbor_graph_rule')) {
		...
		if (!is_error_message()) {
			$rule_id = sql_save($save, 'plugin_neighbor__graph_rules');
			if ($rule_id) 	{ raise_message(1); }
			else 			{ raise_message(2); }
		}

		header('Location: neighbor_graph_rules.php?header=false&action=edit&id=' . (empty($rule_id) ? get_nfilter_request_var('id') : $rule_id));
	}
	elseif (isset_request_var('save_component_neighbor_graph_rule_item')) {
		...
		if (!is_error_message()) {
			$item_id = sql_save($save, 'plugin_neighbor__graph_rule_items');
			if ($item_id) 	{ raise_message(1); }
			else 			{ raise_message(2); }
		}

		if (is_error_message()) {
			header('Location: neighbor_graph_rules.php?header=false&action=item_edit&id=' . get_request_var('id') . '&item_id=' . (empty($item_id) ? get_request_var('item_id') : $item_id) . '&rule_type=' . AUTOMATION_RULE_TYPE_GRAPH_ACTION);
		} else {
			header('Location: neighbor_graph_rules.php?header=false&action=edit&id=' . get_request_var('id') . '&rule_type=' . AUTOMATION_RULE_TYPE_GRAPH_ACTION);
		}
		...
		if (is_error_message()) {
			header('Location: neighbor_graph_rules.php?header=false&action=item_edit&id=' . get_request_var('id') . '&item_id=' . (empty($item_id) ? get_request_var('item_id') : $item_id) . '&rule_type=' . AUTOMATION_RULE_TYPE_GRAPH_MATCH);
		} else {
			header('Location: neighbor_graph_rules.php?header=false&action=edit&id=' . get_request_var('id') . '&rule_type=' . AUTOMATION_RULE_TYPE_GRAPH_MATCH);
		}
	} else {
		raise_message(2);
...
function neighbor_graph_rules_form_actions() {
	...
	if (isset_request_var('selected_items')) {
		...
		if ($selected_items != false) {
			if (get_nfilter_request_var('drp_action') == AUTOMATION_ACTION_GRAPH_DELETE) { /* delete */
				db_execute('DELETE FROM plugin_neighbor__graph_rules WHERE ' . array_to_sql_or($selected_items, 'id'));
				db_execute('DELETE FROM plugin_neighbor__graph_rule_items WHERE ' . array_to_sql_or($selected_items, 'rule_id'));
				db_execute('DELETE FROM plugin_neighbor__match_rule_items WHERE ' . array_to_sql_or($selected_items, 'rule_id'));
				...
				for ($i=0;($i<count($selected_items));$i++) {
					...
					db_execute_prepared("UPDATE plugin_neighbor__graph_rules
						SET enabled='on'
						WHERE id = ?",
	...
	foreach ($_POST as $var => $val) {
		if (preg_match('/^chk_([0-9]+)$/', $var, $matches)) {
			...
			$neighbor_graph_rules_list .= '<li>' . db_fetch_cell_prepared('SELECT name FROM plugin_neighbor__graph_rules WHERE id = ?', array($matches[1])) . '</li>';
...
function neighbor_graph_rules_remove() {
	...
	if ((read_config_option('deletion_verification') == 'on') && (!isset_request_var('confirm'))) {
		top_header();
		form_confirm(__('Are You Sure?'), __("Are you sure you want to delete the Rule '%s'?", db_fetch_cell_prepared('SELECT name FROM plugin_neighbor__graph_rules WHERE id = ?', array(get_request_var('id')))), 'neighbor_graph_rules.php', 'neighbor_graph_rules.php?action=remove&id=' . get_request_var('id'));
		bottom_footer();
		exit;
	}

	if ((read_config_option('deletion_verification') == '') || (isset_request_var('confirm'))) {
		db_execute_prepared('DELETE FROM plugin_neighbor__match_rule_items
			WHERE rule_id = ?
			AND rule_type = ?',
			array(get_request_var('id'), AUTOMATION_RULE_TYPE_GRAPH_MATCH));

		db_execute_prepared('DELETE FROM plugin_neighbor__graph_rule_items
			WHERE rule_id = ?',
			array(get_request_var('id')));

		db_execute_prepared('DELETE FROM plugin_neighbor__graph_rules
			WHERE id = ?',
			array(get_request_var('id')));
	}
}
...
function neighbor_graph_rules_edit() {
	...
	if (!isempty_request_var('id')) {
		$rule = db_fetch_row_prepared('SELECT * FROM plugin_neighbor__graph_rules where id = ?', array(get_request_var('id')));
...
function neighbor_graph_rules() {
						...
						<td>
							<select id='snmp_query_id'>
								...
								$available_data_queries = db_fetch_assoc('SELECT DISTINCT
									sq.id, sq.name
									FROM plugin_neighbor__graph_rules AS ar
	...
	$total_rows = db_fetch_cell("SELECT
		COUNT(agr.id)
		FROM plugin_neighbor__graph_rules AS agr
		LEFT JOIN snmp_query AS sq
